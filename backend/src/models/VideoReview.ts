...
export interface IReviewStep {
  ...
  mentionIds: mongoose.Types.ObjectId[]; // who is asked to act next (one or more)
  mentionNames?: string[];
  seenBy: IReviewSeen[];                 // which mentioned people opened this step
  createdAt: Date;
}
...
export interface IVideoReview extends Document {
  ...
  currentMentionIds: mongoose.Types.ObjectId[]; // people currently asked to act
  steps: IReviewStep[];
...
const VideoReviewSchema = new Schema<IVideoReview>({
  ...
  currentMentionIds: [{ type: Schema.Types.ObjectId, ref: 'User' }],
